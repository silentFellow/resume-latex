% https://www.overleaf.com/latex/templates/modern-cv-template/kmkzxwnmvmqq

\input{setup/preamble}
\input{setup/macros.tex}

\begin{document}
        %%% TItle %%%
        \tcbset{colframe=title,colback=title,arc=0mm}
        \begin{tcolorbox}

            \begin{minipage}{0.3\textwidth} % Picture Area
                \includegraphics[width=0.8\textwidth]{profile.jpg} % Picture
            \end{minipage} \hfill
            \begin{minipage}{0.65\textwidth} % Name and Contact Info
                \name{Gowtham.S}{\small Tech Enthusiast | Passionate about Linux, CLI, and Open-Source | Curious Mind with a Professional Approach to Problem-Solving | Enthusiast in Deep Learning}
                \vspace{1em}
                \email{gowtham.sri@zohomail.in} $\cdot$
                \phone{+91 739-58-31324} \par \vspace{0.5em}
                \address{Tamil Nadu, India} $\cdot$

                \vspace{1em}
                \newline
                  \newcommand{\faLeetCode}{\raisebox{-0.2ex}{\includegraphics[height=1em]{leetcode.png}}}
                  \href{https://github.com/silentFellow}{\faGithub} \hspace{0.5em} | \hspace{0.5em}
                  \href{https://www.linkedin.com/in/silentFellow}{\faLinkedin} \hspace{0.5em} | \hspace{0.5em}
                  \href{https://gowthams-portfolio.vercel.app}{\faGlobe} \hspace{0.5em} | \hspace{0.5em}
                  \href{https://leetcode.com/u/silentFellow}{\faLeetCode}
                \par

            \end{minipage}

        \end{tcolorbox}

    %%% Sections %%%
    \vspace*{-1em}
    \tcbset{colframe=white,colback=white,arc=0mm, height=0.8\textheight}
    \begin{tcolorbox}
        \vspace*{-0.5em}
        \begin{minipage}[t]{0.3\textwidth} % Side Panel (e.g. Skills, Links, Languages, etc.)
            \begin{tcolorbox}[height=0.8\textheight, grow to left by=0.6cm,colback=backdrop,colframe=backdrop,arc=0mm]

                % Skills, the skill level is drawn as bars, input: skill name and an array starting from 0 and ending before 4
                \subsection*{Tech Stack}
                    \skill{Python}{0, 1, 2, 3}
                    \skill{Golang}{0, 1, 2, 3}
                    \skill{JavaScript/TypeScript \newline \textit \textit{(React, NextJs, NodeJs)}
}{0, 1, 2,3}
                    \skill{C, C++, Java}{0, 1, 2,}

                \subsection*{Tech Toolbox}
                    {\LaTeXraggedright
                        \cvtag{Linux (Advanced)}
                        \cvtag{Neovim}
                        \cvtag{Git}
                        \cvtag{Pytorch}
                        \cvtag{Polars}
                        \cvtag{Pyspark}
                        \cvtag{MongoDB}
                        \cvtag{PostgresSQL}
                        \cvtag{AWS}
                        \cvtag{Markdown}
                    \par}

                \subsection*{Strengths}
                    {\LaTeXraggedright
                        \cvtag{Curious}
                        \cvtag{Creative}
                        \cvtag{Smart-Working}
                        \cvtag{Leadership}
                        \cvtag{Teamwork}
                    \par}

                \subsection*{Miscellaneous}
                    1. Published Paper on Enhanced Weather Prediction using ML \href{https://github.com/silentFellow/weatherPrediction.git}{(Link)} \newline
                    2. 2025: MongoDB Associate Developer \newline
                    3. 2024: 30-Hour Hackathon Winner \newline
                \end{tcolorbox}
        \end{minipage}
        \begin{minipage}[t]{0.7\textwidth} % Main Panel (e.g. Education, Work Experience)
            \begin{tcolorbox}[grow to right by=0.75cm,height=0.8\textheight,colframe=white,colback=white]


                % Work Experience
                \section*{Work Experience}
                    \work{Freelancing at \href{https://greencollar.ai/}{greencollar.ai}}{Nov 2023 - April 2024}{}{}
                    \work{Internship at greencollar.ai}{May 2024 - Jan 2025}{official website - internal admin access product}{Green Collar Agritech uses advanced algorithms and deep domain expertise to deliver real-time crop-based predictions, optimizing operations and ensuring product quality.}

                % Education
                \section*{Education}
                    \education{B.Tech AIML - 8.84 CGPA}{Sep 2020 - Oct 2024}{Kongu Engineering College}{} \vspace{1em}
                    \education{HSC - 92.03\%}{May 2021}{Navarasam Matric. Hr. Sec. School}{} \vspace{1em}
                    \education{SSLC - 89.80\%}{May 2019}{Navarasam Matric. Hr. Sec. School}{}

                % Publications
                \section*{Projects}
                    \cvproject
                      {Enhanced Weather Prediction Using ML}
                      {https://github.com/silentFellow/weatherPrediction.git}
                      {}
                      {}
                      {A machine learning project aimed at improving weather prediction accuracy. Utilizes advanced algorithms and data processing techniques.}
                      {Polars, Pytorch, Numpy, Streamlit}

                    \vspace{1em}

                    \cvproject
                      {Abstract-UI}
                      {https://github.com/silentFellow/abstract-ui.git}
                      {https://abstract-ui-olive.vercel.app}
                      {https://www.npmjs.com/package/@silentfellow/abstract-ui}
                      {A component library for React and Next.js. It offers reusable UI components to speed up web development.}
                      {React, TypeScript, Tailwind CSS, Storybook}

                    \vspace{1em}

                    \cvproject
                      {NeoBlog}
                      {https://github.com/silentFellow/neoblog.git}
                      {https://neoblog-ruddy.vercel.app}
                      {}
                      {A blogging platform built with Next.js. Features rich text editing, multimedia support, and SEO optimization.}
                      {NextJs, React, TypeScript, Tailwind CSS, Shadcn-UI, PostgresSQL}

                    \vspace{1em}

            \end{tcolorbox}
        \end{minipage}
    \end{tcolorbox}

\end{document}
